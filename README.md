...
所以各仓都挂着一份**指向这里某个 commit** 的 git submodule（`vendor/dev-infra`），
工作树里看到的就是那个 commit 上 `shared/` 的内容，不是另一套机制生成出来的副本。
边界要说准，两件事不一样：
...
（连同 `shared/githooks/pre-commit` 那个钩子）、装 hook 那个脚本，
以及**它们各自的测试**。

**钩子单独一个目录 `shared/githooks/`，不散在 `shared/` 根下**，这是刻意的：
消费仓的 `core.hooksPath` 指着那个目录，所以**放进去的任何一个按钩子命名的文件
...
所以这一层的文件是**直接从这个仓库的 git 对象里检出的**，不是另一套机制
生成出来的副本——消费仓工作树里手改一份，改动进不了消费仓自己的历史（能改的
只有 dev-infra 这边的提交），所以不需要额外一层校验去防「手改副本」。

**执行位是唯一一处需要专门说的地方**：`shared/githooks/pre-commit` 在这个仓库里是
`100755`，这一位跟着 git 对象一起被 submodule 载走，消费仓拿到的就是可执行的那一份。
**这一位曾经丢过**——推文件用的 GitHub Contents API 一律写 100644，没有能直接设
模式的口子，所以它是**人手动打上去的**，一次。以后如果换一条路径改这份文件
（不是走 PR 里的正常提交），先确认那条路没把这一位打回 100644。
